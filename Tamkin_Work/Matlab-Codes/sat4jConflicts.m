...
function plotData(subPlot, nodes, timeValues, statusValues, maxYval, totalNodes, title1 )
    ...
    for i = 1:totalNodes
       ...
       if(value == 1)
           fc = [154,205,50] ./ 255;
           ec = [107,142,35] ./ 255;
           scatter(subPlot,n,t,s,'filled','MarkerEdgeColor',ec, 'MarkerFaceColor',fc);
       ...
       elseif(value == 3)
           fc = [255,160,122] ./ 255;
           ec = [220,20,60] ./ 255;
           scatter(subPlot,n,t,s,'filleds','MarkerEdgeColor',ec, 'MarkerFaceColor',fc);
           %stem(s1,n,t, 'r', 'filled');
       end   
    end
    
    maxYval = maxYval + (maxYval/6);
    maxXval = totalNodes + 4;
    set(gca, 'Ticklength', [0 0])
    set(gca,'YLim',[-1 *(maxYval/6) maxYval], 'XTick',0:1:maxXval, 'FontSize', 8)
    ylabel('Conflict & Opt,SAT,Un')
