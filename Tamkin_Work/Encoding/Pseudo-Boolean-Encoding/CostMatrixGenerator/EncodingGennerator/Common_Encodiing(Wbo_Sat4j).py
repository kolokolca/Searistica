...
class PBContraintInstanceGenerator:
	...
	def getAdamCostMatrix(self, costMatrixFilePath):
		TravelingTimeMatrix = []
		with open(costMatrixFilePath) as infile:
			numberOfNodes = 0;
			for line in infile:			
				costs = line.split(',')
				intCosts = []
	...
	def getaFileobject(self, outputFilePath):
	...
	def generateObjectiveFunction(self, TravelingTimeMatrix, parentDir):
		string = "";
		self.edge_variable_map = {}
		
		edgeVarMapFilePath = parentDir + "/" + "EdgeVarMapFile.map";
		if(os.path.exists(edgeVarMapFilePath)):
			os.remove(edgeVarMapFilePath)
			
		edgeVarMapFile = self.getaFileobject(edgeVarMapFilePath)
		...
		for i in range(len(TravelingTimeMatrix)):
			...
			for j in range(len(row)):
				...
				string += "+" + cost + " " + edgeVar + " "
				evmStr = edgeKey + " : " + self.edge_variable_map[edgeKey] + "\n"
				edgeVarMapFile.write(evmStr)
		
		edgeVarMapFile.close()
	...
	def getSoubtourEliminationConstraint(self, totalNodes):
		...
		for i in range(1, totalNodes + 1):
			...
			subTourConstraints +=  uiPositiveConstraint
			self.constraintCounter += 1
	...
	def generate(self, options ,givenShortestPaths = None):
		...
		(numberOfNodes, TravelingTimeMatrix) = self.getAdamCostMatrix(options.costMatrixFilePath)
		outPutfile = self.getaFileobject(options.outputFilePath)
		
		parentDir = os.path.dirname(options.costMatrixFilePath)
		objectiveFunction = self.generateObjectiveFunction(TravelingTimeMatrix, parentDir)
		...
		outPutfile.write(PBstring)
		self.closeFileobject(outPutfile)
...
if __name__ == '__main__':
	parser = OptionParser()
	parser.add_option("--costMatrixFilePath", dest="costMatrixFilePath", help="Cost Matrix File path", type="string", default  = 'EuclideanDistanceCostmatrix.txt' )
	parser.add_option("--outputFilePath", dest="outputFilePath", help="output file path", type="string" , default  = 'PBEncoding_From_EuclideanDistanceCostmatrix.opb')
	
	(options, args) = parser.parse_args()
	
	print "------"
	benchMarkFolder = "../BenchMarks";
	costFiles = ['EuclideanCost.txt','OceanDataCost.txt','RandomCost.txt']	
	for folderName in os.listdir(benchMarkFolder):
		#print folderName
		#if(str(folderName) != "4_Nodes"): continue;
		absFolderName = benchMarkFolder + "/" + folderName 
		if(os.path.isdir(absFolderName)):
			print folderName
			for costFile in costFiles:
				costFilePath = absFolderName + "/" + costFile
				if(os.path.exists(costFilePath)):					
					options.costMatrixFilePath = costFilePath;
					parentDir = os.path.dirname(options.costMatrixFilePath)
					costFilename =  os.path.basename(options.costMatrixFilePath).split('.')[0]
					options.outputFilePath = parentDir +  "/PBencd_" + costFilename + ".opb"
					if(os.path.exists(options.outputFilePath)):
						os.remove(options.outputFilePath)
					print "Generating Encoding: " + options.outputFilePath
					generator = PBContraintInstanceGenerator()
					generator.generate(options)
	print "------"
	
	#print options.outputFilePath
	#projectFolderPath =  options.outputFilePath
