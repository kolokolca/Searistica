...
CalendarQueue <- setRefClass("CalendarQueue",
	...
	methods = list(
		...
		calculateAverageSeparation = function() {
			...
			sampleCount <- length(sampleValues)
			for (i in 1:sampleCount) {
				add(sampleValues[[1]], FALSE)
				sampleValues[[1]] <- NULL
			}
		...
		changeListSize = function(sizeChange) {
			print(7)
			backupBaseValue <- baseValue
			
			newBucketSize <- calculateAverageSeparation()

			transferContents = function() {
				if (elementCount == 0) {
					position <<- 1
					baseValue <<- backupBaseValue
					listSize <<- listSize * sizeChange
...
#				incrementPosition()
#			}
			if (elementCount > (listSize * 2) && advancePosition == TRUE) {
				increaseQueueSize()
			}
		},
