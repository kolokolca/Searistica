loadMap <- function(fileName, width, height, dimNames) {
	...
	if (length(dimNames) == 10) {
		...
		unprocessedMapData$previousX <- apply(unprocessedMapData, 1, function(row) -1)
		unprocessedMapData$previousY <- apply(unprocessedMapData, 1, function(row) -1)
	}
